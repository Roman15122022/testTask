...
export class MenuOverlay {
  ...
  private readonly actionButton = new ControlButton('Start', 168, 50);
  ...
  constructor() {
    ...
    panel.drawRoundedRect(0, 0, panelWidth, panelHeight, 22);
    panel.position.set(
      (FIELD_BOUNDS.width - panelWidth) / 2,
      (FIELD_BOUNDS.height - panelHeight) / 2,
    );
    ...
    this.messageText = new Text(
      'Collect animals and bring them to the yard.',
      new TextStyle({
        fill: 0xd8e8d0,
        fontFamily: 'Trebuchet MS',
        fontSize: 20,
    ...
    this.actionButton.view.position.set(
      (FIELD_BOUNDS.width - 168) / 2,
      panel.y + panelHeight - 78,
    );
  ...
  show(status: Exclude<GameStatus, 'running'>, onAction: () => void): void {
